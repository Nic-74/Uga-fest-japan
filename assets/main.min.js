...
(function(){
  ...
  if(cta&&!document.getElementById("themeToggle")){
    ...
    const label=(t)=>t==="dark"?'<span class="ic">🌙</span><span>Dark</span>':'<span class="ic">☀️</span><span>Light</span>';
...
(function() {
  ...
  inputs.forEach(input => {
    ...
    // Add better mobile validation feedback
    input.addEventListener('invalid', function(e) {
      ...
      if (!errorMsg || !errorMsg.classList.contains('error-message')) {
        ...
        errorMsg.style.cssText = 'color: var(--brand); font-size: 0.8rem; margin-top: 4px;';
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          entry.target.setAttribute('aria-live', 'off');
        }
      });
    });
...
(function() {
  ...
  // Add swipe gestures for slideshow
  ...
  if (slideshow) {
    ...
    function handleSwipe() {
      ...
      if (Math.abs(diff) > swipeThreshold) {
        if (diff > 0) {
          // Swiped up - could add functionality
        } else {
          // Swiped down - could close slideshow
          if (typeof closeSlideshow === 'function') {
            closeSlideshow();
          }
        }
      }
