// main.min.js - Updated with improved mobile menu and theme toggle
/* Header menu */
const menuBtn=document.querySelector(".menu-btn"),menu=document.getElementById("menu");if(menuBtn){menuBtn.addEventListener("click",()=>{const o=menu.classList.toggle("open");menuBtn.setAttribute("aria-expanded",o?"true":"false");document.body.style.overflow=o?"hidden":""})}
/* Active nav link */
document.querySelectorAll("nav a").forEach(a=>{const href=a.getAttribute("href")||"",path=location.pathname.split("/").pop()||"index.html";if(href===path||(href==="../index.html"&&path==="")){a.setAttribute("aria-current","page");a.classList.add("active")}});
/* FAQs */
document.querySelectorAll(".faq-q").forEach(q=>{q.addEventListener("click",()=>{const a=q.parentElement.querySelector(".faq-a"),o=a.style.display==="block";a.style.display=o?"none":"block";q.setAttribute("aria-expanded",o?"false":"true");q.lastElementChild.textContent=o?"+":"–"})});
/* Back to top */
...
(function(){
  ...
  if(cta&&!document.getElementById("themeToggle")){
    ...
    const label=(t)=>t==="dark"?'<span class="ic">☀️</span>':'<span class="ic">🌙</span>';
    ...
    update();cta.insertBefore(btn, cta.firstChild);
  }
})();
...
(function() {
  ...
  function applyTranslation(lang) {
    loadTranslations(function(translations) {
      ...
      if (toggleBtn) {
        if (lang === 'ja') {
          toggleBtn.innerHTML = '<span class="ic">🌐</span> EN';
          toggleBtn.setAttribute('aria-label', 'Switch to English');
        } else {
          toggleBtn.innerHTML = '<span class="ic">🌐</span> JA';
